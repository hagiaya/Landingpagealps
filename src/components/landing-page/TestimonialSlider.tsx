...
export default function TestimonialSlider({ testimonials }: TestimonialSliderProps) {
  ...
  // Create an array of testimonials to display in the current view
  let displayedTestimonials = testimonials.slice(startIndex, endIndex);
  
  // If we're near the end and need more items to fill the visible count, loop back to the beginning
  if (displayedTestimonials.length < visibleCount && testimonials.length > visibleCount) {
    const remaining = visibleCount - displayedTestimonials.length;
    displayedTestimonials = [...displayedTestimonials, ...testimonials.slice(0, remaining)];
  }
  ...
  // Previous and next functions for manual navigation
  const goToPrevious = () => {
        ...
        <div className="relative">
          ...
          >
            {displayedTestimonials.map((testimonial, index) => (
              <Card key={`${testimonial.id}-${startIndex + index}`} className="p-6 flex flex-col">
